...
class EntityAttributesInputValueAnyOf1(BaseModel):
    """
    ...
    """ # noqa: E501
    op: StrictStr = Field(description="The operation. `increment` adds `value` (negative to subtract).")
    value: Union[StrictFloat, StrictInt] = Field(description="The operand, as a JSON number.")
    __properties: ClassVar[List[str]] = ["op", "value"]

    @field_validator('op')
    def op_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['increment']):
            raise ValueError("must be one of enum values ('increment')")
        return value
