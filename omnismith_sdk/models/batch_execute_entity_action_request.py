...
class BatchExecuteEntityActionRequest(BaseModel):
    """
    The records to run the action on and the values for its fields, shared by every record.
    """ # noqa: E501
    entity_ids: List[UUID] = Field(description="Distinct entity identifiers, at most 100 per call. Every record must belong to a template that defines the action.")
    ...
    atomic: Optional[StrictBool] = Field(default=False, description="When false (the default) every record is attempted and its outcome reported. When true the batch runs in one transaction and the first record that does not execute rolls all of it back.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["entity_ids", "values", "atomic"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "atomic": obj.get("atomic") if obj.get("atomic") is not None else False
        })
