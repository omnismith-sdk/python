...
class AccessTokensApi:
    ...
    def create_access_token(
        ...
        _response_types_map: Dict[str, Optional[str]] = {
            '201': "CreateAccessToken201Response",
            '400': "ErrorResponse",
            '401': "ErrorResponse",
            '403': "ErrorResponse",
    ...
    def create_access_token_with_http_info(
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[CreateAccessToken201Response]:
        """Create a programmatic API access token
    ...
    def delete_access_token(
        self,
        id: Annotated[UUID, Field(description="Unique UUIDv7 identifier of the access token to delete")],
        x_omnismith_project_id: Annotated[Optional[UUID], Field(description="The project this call acts on. A credential proves identity and grants a set of projects; it never selects one, so every tenant-scoped call names its target here. The value must be one of the projects in the credential's `projects` claim and must still be reachable — a project the caller is not a member of, or one that has been deleted, is rejected with 403 rather than silently ignored. A project the caller *is* a member of but which the credential predates is also rejected with 403, carrying the code `stale_project_grant`; that one is answered by refreshing the credential once and retrying, and is the only 403 here worth retrying. Omitting the header is not an error: the caller is simply acting with no project selected, and a tenant-scoped operation then answers 409 `no_project_selected`. Two clients holding the same credential may send different values at the same time.")] = None,
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
        ...
        :param id: Unique UUIDv7 identifier of the access token to delete (required)
        :type id: UUID
        :param x_omnismith_project_id: The project this call acts on. A credential proves identity and grants a set of projects; it never selects one, so every tenant-scoped call names its target here. The value must be one of the projects in the credential's `projects` claim and must still be reachable — a project the caller is not a member of, or one that has been deleted, is rejected with 403 rather than silently ignored. A project the caller *is* a member of but which the credential predates is also rejected with 403, carrying the code `stale_project_grant`; that one is answered by refreshing the credential once and retrying, and is the only 403 here worth retrying. Omitting the header is not an error: the caller is simply acting with no project selected, and a tenant-scoped operation then answers 409 `no_project_selected`. Two clients holding the same credential may send different values at the same time.
        :type x_omnismith_project_id: UUID
        :param _request_timeout: timeout setting for this request. If one
    ...
    def _delete_access_token_serialize(
        ...
        return self.api_client.param_serialize(
            method='DELETE',
            resource_path='/access-tokens/{id}',
            path_params=_path_params,
            query_params=_query_params,
            header_params=_header_params,
            body=_body_params,
    ...
    def list_access_tokens(
        ...
        _response_types_map: Dict[str, Optional[str]] = {
            '200': "ListAccessTokens200Response",
            '401': "ErrorResponse",
            '403': "ErrorResponse",
            '409': "ErrorResponse",
        }
    ...
    def list_access_tokens_with_http_info(
        ...
        _headers: Optional[Dict[StrictStr, Any]] = None,
        _host_index: Annotated[StrictInt, Field(ge=0, le=0)] = 0,
    ) -> ApiResponse[ListAccessTokens200Response]:
        """List API access tokens
