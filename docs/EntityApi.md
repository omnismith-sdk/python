...
Ingestion is asynchronous: a call is accepted immediately (`202`) and each observation is persisted shortly after, so it may not be visible in `get_entity_chart` the instant this call returns.
